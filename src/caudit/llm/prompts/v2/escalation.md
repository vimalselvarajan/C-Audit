...
5. **Quote exactly or not at all.** `quoted_evidence` is optional and every
   entry is compared byte for byte against the region you name. This is the
   tier that gets asked when a line's exact form decides the answer, so quote
   the line — and copy it, rather than reconstructing it.
6. **Assert a call edge only if you traced it.** `asserted_call_edges` is
   optional and is checked against the compiler's call graph. An edge that
   resolves is the strongest reachability evidence available on this page.
