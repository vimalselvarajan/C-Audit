...
## Current state: parts 01–13 implemented (M1 complete; M2 measured once; M3 infrastructure built but not validated)
...
The mini-suite comparison is **0.6667 → 0.5000**, and `baseline_floor` fails, so `caudit compare` refuses to call it a result. Read that narrowly before concluding anything about the model. On a fixed candidate set, adjudication can improve confirmed-finding precision and recall by classifying analyzer candidates that deterministic promotion left unresolved; it cannot find a defect for which candidate generation produced nothing. This six-case suite has *zero* baseline false positives, so it is a regression check rather than evidence of model quality. The −0.1667 is one case where Gemini answered CWE-120 for an unbounded `strcpy`, which is the canonical mapping and simply outside our 37-entry allowlist; the gate correctly routed it to `out_of_scope_family`. Whether CWE-120 belongs in `ALLOWLIST` is an open decision, deliberately not patched in mid-measurement.
